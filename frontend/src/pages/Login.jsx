...
function useSequentialTyper(lines, charDelay = 38) {
    ...
    useEffect(() => {
        ...
        if (lines[currentLine]?.isInput) {
            setCurrentLine(c => c + 1);
            return;
        }

        const text = lines[currentLine].text;
        let i = lineStates[currentLine].length;

        if (i >= text.length) {
            const t = setTimeout(() => {
                if (!stoppedRef.current) setCurrentLine(c => c + 1);
            }, 120);
        ...
        const t = setTimeout(() => {
            if (!stoppedRef.current) {
                setLineStates(prev => {
                    const next = [...prev];
                    next[currentLine] = text.slice(0, i + 1);
                    return next;
                });
            }
        }, charDelay);
        return () => clearTimeout(t);
    }, [currentLine, lineStates, lines]);
...
const LOGIN_FIELDS = [
    { key: "email", prompt: "enter your email address", type: "email", placeholder: "you@example.com" },
    { key: "password", prompt: "enter your password", type: "password", placeholder: "••••••••••••" },
];
...
export default function LoginPage() {
    ...
    const HEADER_LINES = [
        { text: "// authenticate_user.sh", color: "rgba(59,130,246,0.45)" },
        { text: "session@ai-interview ~ login", color: "rgba(100,116,139,0.6)" },
        { text: "", color: "" },
        { text: "$ initializing secure channel...", color: "rgba(100,116,139,0.55)" },
        { text: "  ✓ connected", color: "rgba(52,211,153,0.7)" },
        { text: "$ loading credentials prompt...", color: "rgba(100,116,139,0.55)" },
    ...
    const currentField = LOGIN_FIELDS[step];
    ...
    const handleKey = (e) => {
        ...
        const updatedFields = { ...fields, [currentField.key]: val }; // Create the final object here
        setFields(updatedFields);
        setHistory(h => [...h, { prompt: currentField.prompt, value: masked }]);
        setFields(f => ({ ...f, [currentField.key]: val }));
        setCurrent("");
        if (step + 1 >= LOGIN_FIELDS.length) {
            setSubmitted(true);
            submit(updatedFields);
            console.log("Login:", { ...fields, [currentField.key]: val });
        } else {
            setStep(s => s + 1);
        }
    };
    ...
    const submit = async (finalData) => {
        try {
            const dataToPost = finalData || fields;
            console.log("Sending to backend:", dataToPost);
            const res = await api.post(`/auth/login`, dataToPost);
            console.log(res);
            if (res.data.success) {
    ...
    return (
      ...
      `}</style>
            ...
            <div className="relative hidden lg:flex flex-col justify-between" style={{ width: "48%", borderRight: "1px solid rgba(59,130,246,0.08)" }}>
                    ...
                    {[
                        ...
                        <circle key={i} cx={d.cx} cy={d.cy} r={d.r}
                            ...
                            style={{
                                ...
                                animation: `dotPulse ${d.dur} ease-in-out ${d.delay} infinite`
                            }} />
                    ))}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 px-12 text-center">
                    ...
                    <p className="text-slate-500 text-sm leading-relaxed max-w-[280px]">
                        AI-powered mock interviews. Instant feedback. Track your growth across every session.
                    </p>
                </div>
            ...
            <div className="relative flex-1 flex flex-col" style={{ background: "rgba(3,6,16,0.98)" }}>
                ...
                <div className="flex-1 flex flex-col justify-center px-10 lg:px-16 pb-16 gap-2"
                    onClick={() => inputRef.current?.focus()}>

                    {HEADER_LINES.map((line, i) => {
                        if (line.text === "") return <div key={i} className="h-2" />;
                        const done = headerStates[i].length === line.text.length;
                        const isActive = !done && headerStates[i].length > 0;
                        return (
                                ...
                                <>
                                    ...
                                    <div className="fi flex items-center gap-3">
                                        ...
                                        <span className="mono text-[10px] tracking-wider" style={{ color: "rgba(71,85,105,0.8)" }}>
                                            step {step + 1} of {LOGIN_FIELDS.length} — press Enter
                                        </span>
                                    </div>
                                </>
                            )}
